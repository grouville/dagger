...
func (s *moduleSchema) moduleSource(ctx context.Context, query *core.Query, args moduleSourceArgs) (*core.ModuleSource, error) {
	...
	case core.ModuleSourceKindLocal:
		if filepath.IsAbs(parsed.modPath) {
			...
			relPath, err := crossplatformRelativePath(cwdStat.Path, parsed.modPath)
			if err != nil {
...
// crossplatformRelativePath computes a relative path between the current working directory
// and modPath without relying on OS-specific separators. This is necessary because the code
// runs inside a Linux container, but the user might have specified a Windows-style modPath.
func crossplatformRelativePath(cwdPath, modPath string) (string, error) {
