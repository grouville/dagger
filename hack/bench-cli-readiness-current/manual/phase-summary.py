...
assert cohort.parent == Path('/tmp') and cohort.name.startswith('dagger-rust-cli-readiness-ab-')
...
for r in report['profiles']:
    ...
    point['info_calls']=[dict(start_ns=s['startNs'],end_ns=s['endNs'],ms=(s['endNs']-s['startNs'])/1e6,
        status=s.get('status'),description=s.get('statusDescription'),attrs=s.get('attrs',{})) for s in infos]
