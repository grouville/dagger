#!/usr/bin/env python3
"""Three matched current-main readiness CLI pairs on one verified-stream engine.
...
CLIS = {'A': Path('/tmp/dagger-cli-runewidth.lhU4UO4i/dagger-candidate'),
        'B': HERE / 'dagger-candidate'}
...
HASHES = {
    CLIS['A']: 'ff1ef25902a037b41e8b16372fcdb806c0cffdd1568a0b9ed75b4e1ef1631f51',
    CLIS['B']: 'd4eb2c81edc8c04a9512d276dce1346f73e45dbc608e9812662623b90e694f86',
    HERE / 'build-inputs.sha256': '28a1d8f7293394b73ab13776dbbb14bb5bfbe4f0a9f9585f3f9606c1d8c9f6a3',
    HERE / 'after.log': '076700faf4dd4208ee2c78b7ab8d28b93b34be1dd43d472cdb50058ebe244586',
...
def main():
    ...
    cli_source = Path('/tmp/dagger-cli-runewidth.lhU4UO4i/source')
    assert subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=cli_source, text=True).strip() == 'f7dc1f5a418408c3a3e772eeaad31863acd85347'
    assert not subprocess.check_output(['git', 'status', '--porcelain'], cwd=cli_source, text=True).strip()
    subprocess.run(['sha256sum', '--check', '/tmp/dagger-cli-runewidth.lhU4UO4i/build-inputs.sha256'], cwd=cli_source, check=True)
    candidate_source = HERE / 'source'
    assert subprocess.check_output(['git','rev-parse','HEAD'],cwd=candidate_source,text=True).strip() == 'f7dc1f5a418408c3a3e772eeaad31863acd85347'
    expected_changes = {' M internal/buildkit/client/client.go', '?? internal/buildkit/client/wait_test.go', '?? internal/buildkit/client/wait_behavior_test.go'}
    assert set(subprocess.check_output(['git','status','--porcelain'],cwd=candidate_source,text=True).splitlines()) == expected_changes
    subprocess.run(['sha256sum','--check',str(HERE/'build-inputs.sha256')],cwd=candidate_source,check=True)
    ...
    for line in (HERE/'build-inputs.sha256').read_text().splitlines():
        ...
        HASHES[candidate_source/relative] = expected
    cohort = Path(tempfile.mkdtemp(prefix='dagger-rust-cli-readiness-ab-'))
    meta = dict(status='running', order='ABBAAB', comparison=True,
                scope='Three matched control-vs-readiness CLI pairs AB/BA/AB on the identical validated stream engine and official gzip Rust image, default registry configuration. Fresh independent Cargo/engine states; three repeated source edits and one actual bstr upgrade each. First-check/dependency timings profiled; other warm timings omit --profile but export local OTLP. Not full installation.',
                cli_paths={side: str(path) for side, path in CLIS.items()},
                cli_sha256={side: HASHES[path] for side, path in CLIS.items()},
                cli_source=str(cli_source), cli_source_commit='f7dc1f5a418408c3a3e772eeaad31863acd85347',
                candidate_cli_source=str(candidate_source), candidate_cli_base='f7dc1f5a418408c3a3e772eeaad31863acd85347',
                cli_upstream_main='7c35e6274737acff0f6bd76614abb5e04efa7d12',
                note='Both CLIs based on main7c35 reverified before capture. B reconciles old098a per-Info WaitForReady behavior and preserves local cancellation causes. Production gRPC configuration and server-Unavailable retry interval unchanged, but queued transport failures now follow gRPC backoff without periodic application ResetConnectBackoff. Same validated stream engine image on both sides. Default registry route, no custom config or prewarming. Docker/CLI/engine images/module/native image preinstalled; host page/CDN caches not purged.',
    ...
    receiver = subprocess.Popen([str(RECEIVER), '-addr', '127.0.0.1:43239', '-out', str(RAW)],
