"""Prepared one-command, local-only lock-write lifecycle diagnostic.
...
p.add_argument('--trial', default='service-lock-web-finite-v1')
args = p.parse_args()
assert '/' not in args.trial and args.trial not in ('', '.', '..')
cases = [('native-web-finite', ['-m', './module', 'api', 'call', 'web', 'hostname'], None)]
...
(out / 'provenance.json').write_text(json.dumps({
    ...
    'boundary': 'one supplemental behavioral diagnostic on retained engine; all profiled; fresh workspace locks, not cold engine/image claims',
