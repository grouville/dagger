"""Prepared five-command, local-only lock-write lifecycle diagnostic.

Each command gets a fresh Git workspace with no lock. The image tag, module
source, input, config, CLI and engine are identical. No digest is supplied.
Run only after the parent grants the exclusive runtime slot.
"""
...
p.add_argument('--cli', type=Path, default=Path('/tmp/collections-perf/sparse-export/production/dagger-production'))
p.add_argument('--trial', default='service-lock-write-v1')
args = p.parse_args()
assert '/' not in args.trial and args.trial not in ('', '.', '..')
cases = [
    ('native-finite', ['-m', './module', 'api', 'call', 'image'], None),
    ('native-web-finite', ['-m', './module', 'api', 'call', 'web', 'hostname'], None),
    ('up-immediate-cancel', ['up', 'web'], 0),
    ('up-delayed-cancel', ['up', 'web'], 1),
    ('core-finite-control', ['-m', 'core', 'api', 'call', 'container', 'from', '--address', 'busybox:1.37', 'image-ref'], None),
]
if not args.run:
    print(json.dumps({'status': 'prepared, not run', 'command_count': len(cases),
                      'cases': cases, 'profiles': 'one per diagnostic command; not performance samples',
                      'workspace': str(args.workspace), 'telemetry': 'local only',
                      'mutation': 'fresh copied Git fixtures only; ordinary lock writes, no manual pin'}, indent=2))
    raise SystemExit()
...
module = inputs['module/main.dang'].decode().rstrip()
assert module.endswith('}') and 'pub image(' not in module
inputs['module/main.dang'] = (module[:-1] + '''
  """Finite counterpart of the image lookup used by the service."""
  pub image(ws: Workspace!): String! {
    container.from("busybox:1.37").imageRef
  }
}
''').encode()
expected_body = inputs['input.txt']
...
def run(label, suffix, alive_seconds):
    dest = out / label
    ws = dest / 'workspace'
    ...
    subprocess.run(['git', '-c', 'user.name=Dagger performance fixture', '-c', 'user.email=perf-fixture@localhost',
                    ...
                    '-m', 'Fresh synthetic lock lifecycle fixture'], cwd=ws, check=True)
    before = lock_state(ws)
    assert not any(item['exists'] for item in before), before
    assert port_closed(), 'fixture port already has a listener'
    dump(dest / 'prior.wcprof')
    command = [str(args.cli), '--engine', 'container://' + engine, '--profile'] + suffix
        ...
        try:
            if alive_seconds is not None:
                ...
                if alive_seconds:
                    ...
                    assert body_matches(), 'service body changed while alive'
    ...
    if alive_seconds is None:
        value = stdout_data.decode().strip()
        if label == 'native-web-finite':
            assert re.fullmatch(r'[A-Za-z0-9][A-Za-z0-9.-]*', value), value
        else:
            image_ref = value
    ...
    row = {'label': label, 'flow': 'up' if alive_seconds is not None else 'image-ref',
           'variant': 'production', 'profile': True, 'command': command, 'workspace': str(ws),
           ...
           'resolved_image_ref': image_ref, 'source_sha256': hashes(ws),
    ...
    assert hashes(ws) == {rel: sha(data) for rel, data in inputs.items()}
...
(out / 'provenance.json').write_text(json.dumps({
    'command_count': len(cases), 'engine': engine, 'cli': str(args.cli), 'cli_sha256': sha(args.cli.read_bytes()),
    ...
    'boundary': 'five behavioral diagnostics on retained engine; all profiled; fresh workspace locks, not cold engine/image claims',
    'same_for_all_cases': 'same module bytes, image tag, input, config, CLI and engine; separate Git roots and lock states',
}, indent=2) + '\n')
try:
    for case in cases:
        run(*case)
