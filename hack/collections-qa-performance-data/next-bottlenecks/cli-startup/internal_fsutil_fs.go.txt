...
func (fs *fs) Walk(ctx context.Context, target string, fn gofs.WalkDirFunc) error {
	var diagVisits, diagDirs int
	if os.Getenv("_DAGGER_CLI_TIMING_DIAG") != "" {
		started := time.Now()
		rootID := sha256.Sum256([]byte(fs.root))
		defer func() {
			fmt.Fprintf(os.Stderr, "CLI_DIAG phase=fsutilWalk root=%x elapsed_us=%d entries=%d directories=%d\n", rootID[:6], time.Since(started).Microseconds(), diagVisits, diagDirs)
		}()
	}
