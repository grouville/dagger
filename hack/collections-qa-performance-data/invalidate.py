...
variants={
 'original':('/tmp/collections-perf/untouched-pr/dagger','container://dagger-engine.collections-untouched',Path('/tmp/greetings-api-collections-perf')),
 'full-stack':('/tmp/collections-perf/dagger-pinned-discovery','container://dagger-engine.collections-profile-stack',Path('/tmp/greetings-api-optimized-perf')),
}
def keys(data):
    out=[]
    ...
    return set(out)
baseline=keys((base/'original-first-use/run-0.out').read_bytes());assert len(baseline)==14
...
try:
 for i,phase in enumerate(phases):
    ...
    assert observed['original']==observed['full-stack'],phase+' order/description/format differs'
