...
base=Path('/tmp/collections-perf/go-existence/greetings/results');base.mkdir(exist_ok=True)
...
variants={'control':('dagger-engine.collections-kyle-syntax',6097),'existence':('dagger-engine.collections-kyle-syntax',6097)}
rows=[]
def run(label,name,profile=False):
 ...
 p=subprocess.run(args,cwd=('/tmp/collections-perf/go-existence/greetings/workspace' if name=='existence' else ws),capture_output=True,text=True)
