...
base=Path('/tmp/collections-perf/next-final/results');base.mkdir(exist_ok=True)
ws='/tmp/collections-perf/kyle-latest/greetings-api';cli='/tmp/collections-perf/committed/dagger';runner='/home/dagger/dag/hack/bench-artifact-discovery.py'
os.environ.pop('SSH_AUTH_SOCK',None)
expected=Path('/tmp/collections-perf/kyle-latest/warm/original-0/run-0.out').read_bytes()
variants={'control':('dagger-engine.collections-kyle-original',6096),'full':('dagger-engine.collections-kyle-syntax',6097)}
rows=[]
def run(label,name,profile=False):
 ...
 args+=['--',('/tmp/collections-perf/untouched-pr/dagger' if name=='control' else cli),'--engine','container://'+engine,'check','-l','--all']
 p=subprocess.run(args,cwd=('/tmp/collections-perf/next-final/workspace' if name=='full' else ws),capture_output=True,text=True)
...
run('setup/full','full')
for name in variants:run('warmup/'+name,name)
for i in range(5):
...
for name in variants:run('profile/'+name,name,True)
