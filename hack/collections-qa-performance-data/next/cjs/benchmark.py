...
base=Path('/tmp/collections-perf/ts-cache/cjs/results');base.mkdir(exist_ok=True)
...
variants={'control':('dagger-engine.collections-kyle-syntax',6097),'cjs':('dagger-engine.collections-kyle-syntax',6097)}
rows=[]
def run(label,name,profile=False):
 ...
 p=subprocess.run(args,cwd=('/tmp/collections-perf/ts-cache/cjs/workspace' if name=='cjs' else ws),capture_output=True,text=True)
