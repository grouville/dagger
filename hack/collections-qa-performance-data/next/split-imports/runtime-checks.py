...
base=Path('/tmp/collections-perf/ts-cache/split-imports/runtime-checks');base.mkdir(exist_ok=True)
os.environ.pop('SSH_AUTH_SOCK',None)
cli='/tmp/collections-perf/committed/dagger';engine='container://dagger-engine.collections-kyle-syntax'
rows=[];nonce=uuid.uuid4().hex
for variant,ws in [('control',Path('/tmp/collections-perf/kyle-latest/greetings-api')),('split',Path('/tmp/collections-perf/ts-cache/split-imports/workspace'))]:
