...
base=Path('/tmp/collections-perf/ts-cache/split-imports/results');base.mkdir(exist_ok=True)
...
variants={'control':('dagger-engine.collections-kyle-syntax',6097),'split':('dagger-engine.collections-kyle-syntax',6097)}
rows=[]
def run(label,name,profile=False):
 ...
 p=subprocess.run(args,cwd=('/tmp/collections-perf/ts-cache/split-imports/workspace' if name=='split' else ws),capture_output=True,text=True)
