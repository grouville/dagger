...
base=Path('/tmp/collections-perf/ts-cache/bun/results');base.mkdir(exist_ok=True)
...
variants={'control':('dagger-engine.collections-kyle-syntax',6097),'bun':('dagger-engine.collections-kyle-syntax',6097)}
rows=[]
def run(label,name,profile=False):
 ...
 p=subprocess.run(args,cwd=('/tmp/collections-perf/ts-cache/bun/workspace' if name=='bun' else ws),capture_output=True,text=True)
