...
import subprocess,sys,json,shlex,uuid,shutil
base=Path('/tmp/collections-perf/committed/greetings')
runner='/home/dagger/dag/hack/bench-artifact-discovery.py'
variants={
 'original':('/tmp/collections-perf/untouched-pr/dagger','container://dagger-engine.collections-committed-original-0',Path('/tmp/collections-perf/committed/greetings/warm-workspace-original')),
 'candidate':('/tmp/collections-perf/committed/dagger','container://dagger-engine.collections-committed-candidate-0',Path('/tmp/collections-perf/committed/greetings/warm-workspace-candidate')),
}
def keys(data):
    out=[]
    ...
    return set(out)
baseline=keys((base/'warmup/original/run-0.out').read_bytes());assert len(baseline)==14
...
try:
 for i,phase in enumerate(phases):
    ...
    assert observed['original']==observed['candidate'],phase+' order/description/format differs'
