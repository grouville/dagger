...
base=Path('/tmp/collections-perf/prebuilt-ts-sdk');blobs=base/'blobs';blobs.mkdir(exist_ok=True)
original=Path('/tmp/collections-perf/ts-cache')
data=io.BytesIO()
with tarfile.open(fileobj=data,mode='w',format=tarfile.PAX_FORMAT) as tar:
    for name,content in [('runtime/.wh.dagger.json',b''),('runtime/dagger-module.toml',Path('/home/dagger/dag/sdk/typescript/runtime/dagger-module.toml').read_bytes()),('bin/typescript-sdk-runtime',(base/'typescript-sdk-runtime').read_bytes())]:
        info=tarfile.TarInfo(name);info.size=len(content);info.mode=0o755 if name.startswith("bin/") else 0o644;info.mtime=0;tar.addfile(info,io.BytesIO(content))
def save(data):
...
config=json.loads((original/'sdk-config.json').read_text());config['rootfs']['diff_ids'].append(layer['digest']);config['history'].append({'created_by':'Prebuild the fixed TypeScript SDK Go runtime'})
...
engine='dagger-engine.collections-prebuilt-ts-sdk';port=6114
...
subprocess.run(['docker','cp','/tmp/collections-perf/prebuilt-ts-sdk/engine',engine+':/usr/local/bin/dagger-engine'],check=True)
...
while True:
    try:
        ...
        time.sleep(.1)
(base/'manifest.json').write_text(json.dumps({'engine':engine,'port':port,'sdk_manifest':new,'added_layer':layer,'binary':'/tmp/collections-perf/prebuilt-ts-sdk/engine'},indent=2)+'\n')
