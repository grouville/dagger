...
base=Path('/tmp/collections-perf/ts-runtime-manifest')
manifest=json.loads((base/'manifest.json').read_text())
expected=Path('/tmp/collections-perf/kyle-latest/warm/original-0/run-0.out').read_bytes()
rows=[]
for variant,port in [('candidate',6111),('control',6112)]:
    name='dagger-engine.collections-manifest-reverse-'+variant
    ...
    if variant=='candidate':cmd+=['-e','DAGGER_TYPESCRIPT_SDK_MANIFEST_DIGEST='+manifest['sdk_manifest']['digest']]
    ...
    if variant=='candidate':
        for f in (base/'blobs').iterdir():subprocess.run(['docker','cp',str(f),name+':/usr/local/share/dagger/content/blobs/sha256/'+f.name],check=True)
    ...
    while True:
        try:
            ...
            time.sleep(.1)
    dest=base/'reverse'/variant
    cmd=[sys.executable,'/home/dagger/dag/hack/bench-artifact-discovery.py','--runs','1','--warmups','0','--timeout','600','--output',str(dest),'--wcprof-url',f'http://127.0.0.1:{port}','--cold-profile','--','/tmp/collections-perf/committed/dagger','--engine','container://'+name,'check','-l','--all']
    ...
    (base/'reverse/results.json').write_text(json.dumps(rows,indent=2)+'\n');print(json.dumps(row),flush=True)
