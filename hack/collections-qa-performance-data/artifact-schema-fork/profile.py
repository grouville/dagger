...
cmd=['/tmp/collections-perf/committed/dagger','--engine','container://dagger-engine.collections-tsx-node-cache','check','-l','--all']
...
run('warmup')
def profile():
 (b/'engine.cpu').write_bytes(urllib.request.urlopen('http://127.0.0.1:6145/debug/pprof/profile?seconds=15',timeout=25).read())
