...
base=Path('/tmp/collections-perf/kyle-latest');ws=base/'greetings-api'
...
(ws/'dagger.toml').write_text(Path('/tmp/collections-perf/committed/greetings/warm-workspace-candidate/dagger.toml').read_text().replace('/tmp/dagger-go-discovery','/tmp/dagger-go-kyle-latest'))
shutil.copy2('/tmp/collections-perf/committed/greetings/warm-workspace-candidate/dagger.lock',ws/'dagger.lock')
variants={
 'original':{'engine':'dagger-engine.collections-kyle-original','binary':'/tmp/collections-perf/untouched-pr/engine','port':6096,'cli':'/tmp/collections-perf/untouched-pr/dagger','fresh':True},
 'syntax-cache':{'engine':'dagger-engine.collections-kyle-syntax','binary':'/tmp/collections-perf/syntax-isolated/engine','port':6097,'cli':cli,'fresh':True},
 'committed':{'engine':'dagger-engine.collections-committed-candidate-0','binary':'/tmp/collections-perf/committed/engine','port':6091,'cli':cli,'fresh':False},
}
...
def measure(label,name,profile=False):
 ...
 args+=['--',v['cli'],'--engine','container://'+v['engine'],'check','-l','--all']
...
for i in range(1):
 ...
 # Original baseline retains the original CLI.
 s=s.replace('dagger-engine.collections-committed-original-0',variants['original']['engine']).replace('dagger-engine.collections-committed-candidate-0',variants['syntax-cache']['engine'])
