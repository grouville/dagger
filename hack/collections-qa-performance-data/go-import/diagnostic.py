...
rows=[]
for variant,port in [('candidate',6120)]:
    name='dagger-engine.collections-go-import-profiled'
    ...
    subprocess.run(['docker','cp',str(base/'engine') if variant=='candidate' else '/tmp/collections-perf/rebuilt-prototypes/engine',name+':/usr/local/bin/dagger-engine'],check=True)
    if True:
    ...
    while True:
        try:
            ...
            time.sleep(.1)
    dest=base/'diagnostic'/variant
    dest.mkdir(parents=True,exist_ok=True)
    import concurrent.futures, urllib.request
    pool=concurrent.futures.ThreadPoolExecutor(max_workers=1)
    def cpu_profile():
        with urllib.request.urlopen(f'http://127.0.0.1:{port}/debug/pprof/profile?seconds=30',timeout=65) as r:
    ...
    (base/'diagnostic/results.json').write_text(json.dumps(rows,indent=2)+'\n');print(json.dumps(row),flush=True)
