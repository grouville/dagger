#!/usr/bin/env python3
"""Complete-trace Unix-readiness engine comparison; invalidation/cold/restart gates retained."""
import argparse
...
def main():
    ...
    assert meta['order'] == 'ABBAAB' and len(meta['runs']) == 6
    assert [sample['side'] for sample in meta['runs']] == list('ABBAAB')
    assert list(sorted(meta['cli_sha256'])) == ['A', 'B']
    ...
    assert meta['image_ids']['A'] == 'sha256:1fb3565bb5bac059d2a89e52e231203befe215ba4396ea40c93393b19f77aeb0'
    assert all(sample['host_inventory_before'] == meta['host_inventory'] == sample['host_inventory_after']
               for sample in meta['runs']), 'host inventory drift is not a performance result'
    ...
    for sample in meta['runs']:
        ...
        assert fixture['engine_provisioning'] == 'ordinary-image-driver-inside-cli'
        assert fixture['debug_helper_sha256'] == 'd50874b7f7c0235bececcb4dbad34655dcadf06452cc3f767d11a9b6160cc5d8'
