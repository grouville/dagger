#!/usr/bin/env python3
"""One standalone, source-validating CAS import diagnostic; not a Cargo scorecard."""
import argparse
...
def main():
    ...
    try:
        ...
        if args.parity_with:
            ...
            assert reference['status'] == 'source-and-profile-validated-diagnostic-only'
        ...
        with (out / 'profile.wcprof').open('xb') as profile:
            subprocess.run(['docker', 'exec', owned['engine']['id'], '/tmp/cas-debug-get', '/debug/wcprof/dump'],
                           env=env, stdout=profile, stderr=subprocess.PIPE, check=True, timeout=30)
        ...
        gates = {
            ...
            'intervals_inside_process_window': bool(ops) and all(
                record['started_ns'] <= header['epoch_unix_nano'] + e['s']
                <= header['epoch_unix_nano'] + e['d'] <= record['process_ended_ns'] for e in events),
        ...
        identity()
        record['status'] = 'source-and-profile-validated-diagnostic-only'
        record['owner'] = owned
