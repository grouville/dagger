#!/usr/bin/env python3
"""Prepare/run a frozen four-arm, seven-flow filesync matrix.
...
ARMS = ('main', 'xattrs', 'cas', 'both')
SEEDS = dict(zip(ARMS, (6, 3, 7, 8)))
HEADS = dict(zip(ARMS, (
    'c4512cf1d1a371ca097d13a311c523638f4c7c0b',
    'd78bcdbf82c92961d5beb6c75252bf10f8b4af1a',
    '69243167b6ee6c1e5c3724045f2e77b8f80928e5',
...
ARM_LABELS = {
    ...
    'both': 'Same main + xattr opt-out + result-first flat CAS',
}
HELPERS = (
    'matrix.py', 'summarize_matrix.py', 'build.py', 'profile_import.py',
...
def cohort_path(number):
    ...
    return ROOT / f'filesync-matrix-r{number}'
...
def position_counts(rounds):
    return {arm: {str(position + 1): sum(row['order'][position] == arm for row in rounds)
                  for position in range(4)} for arm in ARMS}
...
def inspect_seed(arm, number):
    ...
    require(receipt['status'] == 'exported' and receipt['exit_code'] == 0 and build.inventory_compatible(receipt),
...
def prepare(args):
    ...
    require(args.first_attempt >= 2 and args.first_attempt + 23 <= 128, '24 attempts must fit in 2..128; attempt 1 is a legacy runtime')
    ...
    for number in range(1, 7):
        ...
        for offset, arm in enumerate(ARMS):
            attempt = args.first_attempt + (number - 1) * 4 + offset
    ...
    plan = {
        ...
        'scope': 'Four matched filesync arms; no Cargo/module or unprofiled performance claim.',
        'cold': 'New engine volume and CLI XDG state per arm; images and host OS page caches already available.',
        'gc': 'Default configuration; the identical existing reserve-floor fix is present in all four arms. All GC tails retained.',
        'config': {'logLevel': 'debug'}, 'position_counts': position_counts(rounds),
        'order_caveat': 'Six cyclic rounds are not position-balanced across four arms: each arm occupies each position once or twice.',
        ...
        'readback_scope': 'First round, all four arms, edited and moved trees; reimports after all seven timed flows, not retained snapshot-ID readback.',
    }
    output.mkdir()
...
def run(args):
    ...
    try:
            ...
            for position, arm in enumerate(row['order'], 1):
                ...
                try:
                    for label in LABELS:
                        ...
                        command('profile_phase.py', label, '--attempt', attempt, *RELATIONS[label], *parity)
                        observed = state()
                        ...
                        write_new(runtime / label / 'matrix-state.json', {
                            ...
                            'run_sha256': plan['helpers']['matrix.py'], 'moved': label in ('leaf2', 'ab1-exact'),
                        })
                        seen[label] = attempt
                        arm_record['flows'].append(label)
                        save_record(record_path, record)
                    if number == 1:
                        prior = row['arms'][row['order'][0]]['attempt']
                        compare = ['--compare-attempt', prior] if prior != attempt else []
                        command('verify_filesync_export.py', attempt, *compare)
                        require(state() == edit_state, 'edited readback mutated source/mtimes')
                        active_move.move()
                        command('verify_filesync_export.py', attempt, '--capture', 'moved', *compare)
                        require(state() == moved, 'moved readback mutated source/mtimes')
                        active_move.restore()
        ...
        save_record(record_path, record)
    print(json.dumps({'matrix': str(output), 'status': record['status'], 'samples': 6 * 4 * 7}))


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    sub = parser.add_subparsers(dest='action', required=True)
    prep = sub.add_parser('prepare', help='Validate four frozen seeds; create unused runtime folders, never engines')
    prep.add_argument('--cohort', type=int, default=1)
    prep.add_argument('--first-attempt', type=int, default=21)
    execute = sub.add_parser('run', help='Run all seven flows sequentially; preserve evidence on failure')
