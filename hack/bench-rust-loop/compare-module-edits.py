#!/usr/bin/env python3
"""Compare module variants on alternating, novel edits in owned run.py copies.
...
def main():
    ...
    metadata = {
        "purpose": "alternating novel edits; not native Cargo or cold performance",
    ...
    def process(side, scenario, sample, diagnostic=False):
        ...
        argv = [str(dagger)] + (["api", "call", "rust", "check-log"] if diagnostic
                               else ["check", "rust:check"])
    ...
    try:
        ...
        require(digest(dagger) == cli_hash, "CLI changed during comparison")
        summary = {}
