#!/usr/bin/env python3
"""Three matched current-main stream/hash engine pairs on the official Rust image.
...
CLIS = {'A': Path('/tmp/dagger-cli-runewidth.lhU4UO4i/dagger-candidate'),
        'B': Path('/tmp/dagger-cli-runewidth.lhU4UO4i/dagger-candidate')}
...
HASHES = {
    CLIS['B']: 'ff1ef25902a037b41e8b16372fcdb806c0cffdd1568a0b9ed75b4e1ef1631f51',
...
def main():
    ...
    cohort = Path(tempfile.mkdtemp(prefix='dagger-rust-current-stream-ab-'))
    meta = dict(status='running', order='ABBAAB', comparison=True,
                scope='Three matched control-vs-verified-stream/hash engine pairs AB/BA/AB on identical CLI and unchanged official gzip Rust image, default engine registry configuration. Both engines rebuilt from current-main rebase503d with identical local containerd layout; no Dang parser-statistics experiment. Fresh independent Cargo/engine states; three repeated source edits and one actual bstr upgrade each. First-check/dependency timings profiled; other warm timings omit --profile but export local OTLP. Not full installation.',
                ...
                note='CLI and both experimental engines based on latest observed main7c35. Candidate activates combined stream/hash plus the reproduced input-before-decoder cleanup fix and lifecycle/lease prerequisites. The original candidate failed its Zstd early-error gate and was not timed; its failed build and negative tests remain retained. Default registry route: no custom config, mirror override, prepopulated local registry or custom Rust image. Docker/CLI/engine images/module/native image preinstalled; host page/CDN caches not purged.',
