...
Dagger supports private Go modules over HTTPS using credentials from the client's Git credential system. SSH-based access also remains supported by configuring a `.gitconfig` `insteadOf` rule such as `git config --global url."git@github.com:".insteadOf "https://github.com/"`.
</TabItem>
