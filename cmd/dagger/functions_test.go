...
func TestFunctionArgNamedWorkspaceDoesNotShadowGlobalWorkspaceFlag(t *testing.T) {
	...
	require.Same(t, root.PersistentFlags().Lookup("workspace"), flag)
	require.Nil(t, cmd.LocalNonPersistentFlags().Lookup("workspace"))

	require.NoError(t, cmd.ParseFlags([]string{"--workspace", "github.com/acme/workspace"}))
