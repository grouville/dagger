...
func TestChangesetAutoApplyAnalysisErrors(t *testing.T) {
	transportErr := errors.New("fake transport failure")
	tests := []struct {
		name    string
		replies []changesetApplyTestReply
		want    string
		...
		{"ID failure", []changesetApplyTestReply{{body: `{"errors":[{"message":"invalid changeset ID"}]}`}}, "get changeset id", nil},
		{"paths failure", []changesetApplyTestReply{{body: changesetApplyTestIDResponse}, {body: `{"errors":[{"message":"path evaluation failed"}]}`}}, "path evaluation failed", nil},
		{"partial paths with error", []changesetApplyTestReply{{body: changesetApplyTestIDResponse}, {body: `{"data":{"changeset":{"addedPaths":["new.txt"]}},"errors":[{"message":"modifiedPaths failed"}]}`}}, "modifiedPaths failed", nil},
		{"transport failure", []changesetApplyTestReply{{body: changesetApplyTestIDResponse}, {err: transportErr}}, "fake transport failure", transportErr},
		{"cancellation", []changesetApplyTestReply{{body: changesetApplyTestIDResponse}, {err: context.Canceled}}, "context canceled", context.Canceled},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			dag, conn := newChangesetApplyTestClient(t, test.replies...)
			applied, err := handleChangesetResponseWithApply(t.Context(), dag, changesetApplyTestID, changesetDispositionApply, nil,
				func(context.Context, *dagger.Changeset) error {
					t.Fatal("must not apply after failed analysis")
					return nil
				})
			...
			require.Len(t, conn.requests, len(test.replies))
		})
	}
}

func TestChangesetAutoApplyCallbackErrors(t *testing.T) {
	for _, cancelDuringApply := range []bool{false, true} {
		t.Run(fmt.Sprintf("cancel=%t", cancelDuringApply), func(t *testing.T) {
			dag, conn := newChangesetApplyTestClient(t,
				changesetApplyTestReply{body: changesetApplyTestIDResponse},
				changesetApplyTestReply{body: `{"data":{"changeset":{"modifiedPaths":["result.txt"]}}}`},
			)
			...
			calls := 0
			applied, err := handleChangesetResponseWithApply(ctx, dag, changesetApplyTestID, changesetDispositionApply, nil,
				func(applyCtx context.Context, _ *dagger.Changeset) error {
					calls++
					if cancelDuringApply {
						cancel()
						return applyCtx.Err()
...
func TestChangesetAutoApplyInvalidAndNilResponse(t *testing.T) {
	for _, response := range []any{(*dagger.Changeset)(nil), nil, 42, map[string]any{"notID": "x"}} {
		t.Run(fmt.Sprintf("%T", response), func(t *testing.T) {
			dag, conn := newChangesetApplyTestClient(t)
			applied, err := handleChangesetResponseWithApply(t.Context(), dag, response, changesetDispositionApply, nil,
				func(context.Context, *dagger.Changeset) error {
					t.Fatal("must not apply absent or invalid response")
					return nil
				})
			require.False(t, applied)
			if _, typedChangeset := response.(*dagger.Changeset); typedChangeset {
				require.NoError(t, err)
			} else {
				require.ErrorContains(t, err, "unexpected response type for changeset")
			}
			require.Empty(t, conn.requests)
		})
	}
}
