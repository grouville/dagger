...
var vcsPaths = []*vcsPath{
	...
	// 	re:     `^(?P<root>dev\.azure\.com/(?P<account>[A-Za-z0-9_.\-]+)/(?P<project>[A-Za-z0-9_.\-]+)/_git/(?P<repo>[A-Za-z0-9_.\-]+))(/[\p{L}0-9_.\-]+)*(/.*)?$`,
	// 	vcs:    "git",
	// 	repo:   "https://{root}",
	// 	check: func(match map[string]string) error {
	// 		match["repo"] = strings.TrimSuffix(match["repo"], ".git")
	...
	// 	},
	// },
	// In vcs.go, update the Azure DevOps HTTPS entry in vcsPaths:

	{
		...
		check: func(match map[string]string) error {
			if match["project"] == "" {
				match["root"] = strings.TrimSuffix(match["root"], ".git")
			}
			match["repo"] = strings.TrimSuffix(match["repo"], ".git")
			return nil
		},
	...
	// {
	// 	prefix: "ssh.dev.azure.com/",
	// 	re:     `^(?P<root>ssh\.dev\.azure\.com/v\d+/(?P<account>[A-Za-z0-9_.\-]+)/(?P<project>[A-Za-z0-9_.\-]+)/(?P<repo>[A-Za-z0-9_.\-]+)(\.git)?)(/[\p{L}0-9_.\-]+)*(/.*)?$`,
	// 	vcs:    "git",
	// 	repo:   "https://dev.azure.com/{account}/{project}/_git/{repo}",
	// 	check: func(match map[string]string) error {
	// 		match["repo"] = strings.TrimSuffix(match["repo"], ".git")
